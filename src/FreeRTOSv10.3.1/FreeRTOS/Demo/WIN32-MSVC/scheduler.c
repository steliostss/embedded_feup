...
*/
		...
		{
			SchedTCB_t* pxTCBToSuspend = NULL;//( SchedTCB_t * ) pvTaskGetThreadLocalStoragePointer( xTaskHandle, schedTHREAD_LOCAL_STORAGE_POINTER_INDEX );
		...
		void vSchedulerReadyTrace( TaskHandle_t xTaskHandle )
		{
			if( xTaskGetSchedulerState() == taskSCHEDULER_NOT_STARTED || xTaskHandle == xSchedulerHandle )
			{
				...
				SchedTCB_t* pxTCBReady = NULL;//( SchedTCB_t * ) pvTaskGetThreadLocalStoragePointer( xTaskHandle, schedTHREAD_LOCAL_STORAGE_POINTER_INDEX );
	...
	static void prvPollingServerFunction( void )
	{
		for( ; ; )
		{
				...
				else
				{
					...
					SchedTCB_t* pxThisTask = NULL;//( SchedTCB_t * ) pvTaskGetThreadLocalStoragePointer( xTaskGetCurrentTaskHandle(), schedTHREAD_LOCAL_STORAGE_POINTER_INDEX );
